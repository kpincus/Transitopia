...
////////// start of scenario /////////////////
	...
	// d3.selectAll("#testScenario").on("click", function() {	 // changed
		...
		var routes = ["routeJ", "routeU", "routeW"];
		var changes = ["percent10", "percent10", "percent30"];
...
// kp: calc of affected minority percent if rect previously selected		  
      ...
      //Update front-end numbers
        ...
        var diRatio = (100 * minTotal / (popTotal + .01))/41.4; 
		
		//new
		var diRatioBur = (100 * minTotalBur / (popTotalBur + .01))/41.4;
		var diRatioBen = (100 * minTotalBen / (popTotalBen + .01))/41.4;
		...
		d3.selectAll('#isThereBurden')
          .text(function() { 
			...
			else if (diRatio < 0 && diRatioBur < brushValue) {return "No Disproportionate Burden"}
            else if (diRatio > 0 && minTotal < 0 && diRatio < brushValue) { return "No Disproportionate Burden" }
			else if (diRatio > 0 && minTotal > 0) {return "N/A"}
            else { return "Disproportionate Burden" }
          })  
		  
		d3.selectAll('#isThereBenefit')
          .text(function() { 
		    if (diRatioBen == 0) {return "N/A"}
			else if (diRatio < 0 && diRatioBen > 2-brushValue) {return "No Disproportionate Benefit"}
            else if (diRatio > 0 && minTotal > 0 && diRatio > (2-brushValue)) { return "No Disproportionate Benefit" }
			else if (diRatio > 0 && minTotal < 0) {return "N/A"}
...
/////////// end of scenario //////////////////
    ...
    d3.selectAll(".selection").on("mouseover", function() { 
      if($(this).hasClass("clicked") == false){ 
            ...
            d3.selectAll("." + routeName).filter("text").filter(".numRiders, .minPercent, .letterName, .vrhTime")
              .style("font-weight", 700)
              .style("fill", "royalblue") // this was a hashtag
...
CTPS.demoApp.generateMap = function(tracts, routes) {  
  ...
  tractMap.selectAll(".tracts")
      ...
      .append("path")
        ...
        .style("fill", "rgb(182,0,141)") /*change map colors*/
        .style("opacity",  function(d) { return Math.sqrt(d.properties.LOW_INC_HH_PCT);})
  ...
  tractMap.selectAll(".routes")
    ...
    tractMap.append("rect")
      .style("fill", "rgb(182,0,141)").style("stroke", "none").style("opacity", .39)
    ...
    tractMap.append("text")
      ...
      .text("15%");
    tractMap.append("rect")
      .style("fill", "rgb(182,0,141)").style("stroke", "none").style("opacity", .55)
    ...
    tractMap.append("text")
      ...
      .text("30%");
    tractMap.append("rect")
      .style("fill", "rgb(182,0,141)").style("stroke", "none").style("opacity", .67)
    ...
    tractMap.append("text")
      ...
      .text("45%");
    tractMap.append("rect")
      .style("fill", "rgb(182,0,141)").style("stroke", "none").style("opacity", .76)
    ...
    tractMap.append("text")
      ...
      .text("60%");
    tractMap.append("rect")
      .style("fill", "rgb(182,0,141)").style("stroke", "none").style("opacity", .85)
...
CTPS.demoApp.generatePanel = function(source) {
  ...
  source.forEach(function(i){
      ...
      i.Minority_Percent = +i.LI_Percent; // +i.Minority_Percent;	THIS IS THE ONLY THING TO CHANGE!!!
      i.Selected = 0;	  
  })
...
function brushed() {
  ...
  d3.select('.affected')
      .attr("width", xScaleLength(value * 41.4))
  ...
  d3.selectAll('#isThereImpact')
    .text(function() { 
      if (diRatio < brushValue) { return "No Disproportionate Impact" }
      else { return "Disproportionate Impact" }
    })

	d3.selectAll('#isThereBurden')
	  .text(function() { 
		if (diRatioBur == 0) {return "N/A"}
		else if (diRatio < 0 && diRatioBur < brushValue) {return "No Disproportionate Burden"} // copied another instance of this if statement and squashed the bug!
        else if (diRatio > 0 && minTotal < 0 && diRatio < brushValue) { return "No Disproportionate Burden"} 
		else if (diRatio > 0 && minTotal > 0) {return "N/A"}
		else { return "Disproportionate Burden" }
	  })  
...
CTPS.demoApp.generateSavings = function(source) {
  ...
  toggler.append("text")
    .text("Total")
    .attr("x", xLabel + 75).attr("y", yLabel)	// changed from 85 on minority page
    .style("text-anchor", "end").style("font-size", 12).style("font-weight", 700)

  toggler.append("text")
    .text("Ridership")
    .attr("x", xLabel + 75).attr("y", yLabel + 15)
    .style("text-anchor", "end").style("font-size", 12).style("font-weight", 700)
